...
function editarUsuario(id) {
    fetch(`${API}/${id}`) // faz uma requisição GET para pegar o usuário pelo id
        ...
        .then(usuario => { // aqui ele vai preencher os campos do formulário com os dados do usuário
            document.getElementById("id").value = usuario.id_usuario; // preenche o campo id
            document.getElementById("nome").value = usuario.nome_usuario; // preenche o campo nome
            document.getElementById("senha").value = usuario.password; // preenche o campo senha
            document.getElementById("ramal").value = usuario.ramal ? parseInt(usuario.ramal) : 0; // preenche o campo ramal
